...
var _ tunnel.DialerCloser = (*fakeDialer)(nil)

// fakeDialer is a minimal in-memory tunnel.DialerCloser for build tests.
...
func TestBuildDialer(t *testing.T) {
	...
	t.Run("success path with fake builder", func(t *testing.T) {
		...
		fake := &fakeDialer{}
		fn := BuilderFn(func(_ context.Context, parsed *wgconf.ParsedConfig, _ *slog.Logger) (tunnel.DialerCloser, error) {
	...
	t.Run("parse error returns wrapped plain error", func(t *testing.T) {
		...
		called := false
		fn := BuilderFn(func(_ context.Context, _ *wgconf.ParsedConfig, _ *slog.Logger) (tunnel.DialerCloser, error) {
			called = true
			return nil, nil
		})
	...
	t.Run("builder receives correct parsed options", func(t *testing.T) {
		...
		fn := BuilderFn(func(_ context.Context, p *wgconf.ParsedConfig, _ *slog.Logger) (tunnel.DialerCloser, error) {
			capturedParsed = p
