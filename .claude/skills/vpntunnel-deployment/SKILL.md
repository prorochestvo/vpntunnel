...
description: How vpntunnel reaches and lives on the production host — the `/opt/vpntunnel/` release layout with its immutable `artifacts/<VERSION_ID>/` store and `bin/release` channel symlink, the root-owned secret tree that confines the `github_aide` CI identity to two subdirectories, the startup permission assertions, the operator-managed `.env` and systemd unit, the `make init` / `make deploy-nginx` / `make healthz` operator targets, tunnel `.conf` storage, and the pinned gvisor revision. Load before touching `Makefile` deploy targets, `configs/provision-host.sh`, `configs/vpntunnel.service`, `configs/vpntunnel.sudoers`, `configs/env.example`, `configs/nginx/`, `configs/RUNBOOK-migrate-release-layout.md`, `DEPLOY.md`, `.github/workflows/ci.main.yml` or any release workflow; before changing `VERSION_ID`, `bin/release`, `artifacts/`, `EnvironmentFile`, `ExecStart`, `ProtectSystem=strict`, `NoNewPrivileges`, `PrivateTmp`, the `0700`/`0755` modes on `configs/auth`, `configs/tls`, `configs/tunnels`, `state/` or `logs/`, or the `/etc/sudoers.d/vpntunnel-deploy` grant; before reading or writing `VPNTUNNEL_TLS_CERT_DIR`, `VPNTUNNEL_TELEGRAMBOT_DSN`, `VPNTUNNEL_CONFIG_PATH`, `VPNTUNNEL_ADMIN_TOKEN`, `SSH_PRIVATEKEY` or `SSH_HOSTPORT`; and before touching `./configs/tunnels/` or the `gvisor.dev/gvisor` pin in `go.mod`.
---
