...
description: The vpntunnel proxy and API listeners in full — the CONNECT/absolute-URI split on 127.0.0.1:7788, the 407 Proxy Authentication Required challenge and its loopback bypass, the X-Vpntunnel-Token admin/proxy role split on 127.0.0.1:8888, the four frozen v1 route patterns, the break-before-make semantics of /v1/admin/rotate, and the fixed /v1/admin/health body. Load before touching internal/gateway/httpserver, internal/gateway/router, internal/gateway/middleware, internal/gateway/httpV1/routes, internal/gateway/httpV1/handlers, internal/gateway/httpV1/dto, internal/application/proxy.go, ProxyService, HandleHTTP, HandleCONNECT, WaitTunnels, RequestSummary, or internal/infrastructure/ipdeny; before adding, moving or renaming any `/v1/tunnels`, `/v1/tunnels/{id}/proxy/{scheme}/{rest...}`, `/v1/admin/health` or `/v1/admin/rotate` route; and before changing `routes.Tunnels`, `routes.TunnelProxy`, `routes.AdminHealth`, `routes.AdminRotate`, `X-Request-Id`, `X-Proxy-Error`, `Proxy-Authorization`, `?force=true`, `?country=us,se`, or anything that renders `TunnelHealth`.
---
